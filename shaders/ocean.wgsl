...
fn fs(in: VSOut) -> @location(0) vec4f {
  ...
  // The bed is flat over the refraction offset, the same assumption the sand
  // colour above already makes. Biased off the terrain, not off the water:
  ...
  var water = mix(vec3f(0.004, 0.02, 0.05), sand, trans) * lightTint * mix(1.0, occ.bed, SHADOW_WRAP);
  ...
  let foamColor = lightTint * mix(0.45, 1.0, sunLevel) * (0.72 + 0.22 * max(n.y, 0.0)) * mix(1.0, occ.surface, SHADOW_WRAP);
